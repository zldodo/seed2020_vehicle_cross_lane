...
def preprocess(image, max_size=512, mean=(0.485, 0.456, 0.406), std=(0.229, 0.224, 0.225)):
    ori_imgs = [img[..., ::-1] for img in image]
...
def preprocess_one(img, max_size=512, mean=(0.485, 0.456, 0.406), std=(0.229, 0.224, 0.225)):
    # ori_img = img[..., ::-1] 
    ori_img = img
    normalized_img = (img / 255 - mean) / std 
    img_meta = aspectaware_resize_padding(img, max_size, max_size,
                                            means=None) 
...
def write_result_json(res_data,filename,save_path):
    ...
    # print(json_str)
    print(save_path + filename + "_vehicle_info.json")
    with open(save_path + filename + "_vehicle_info.json",'w') as f:
        f.write(json_str)