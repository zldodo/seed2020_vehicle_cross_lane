...
def Testing():
    ...
    if p.mode == 0 : # check model with test data 
        ...
        video_list = os.listdir(p.video_root)
        # print(video_list)
        for video_idx in video_list:
            print(p.video_root+video_idx)
            cap = cv2.VideoCapture(p.video_root+video_idx)
            use_ori = True
            while(cap.isOpened()):
                ...
                if use_ori:
                    w_ratio = p.x_size * 1.0 / ori_frame.shape[1]
                    h_ratio = p.y_size* 1.0 / ori_frame.shape[0]
                    out_x, out_y, ti = test_ori(lane_agent, ori_frame, np.array([frame]), w_ratio, h_ratio,draw_type= 'point',thresh=p.threshold_point)
                else:
        ...
        img_folders = os.listdir(p.img_root)
        for folder in img_folders:
            
            test_images = p.img_root+folder+ '/'
            print("folder index = {}".format(folder))

            # save_test_dir = './test_curves_result'
            save_json_dir = './res_json/test_0.5/'+folder+ '/'
            ...
            for img in img_list:
                ...
                if use_ori:
                    ...
                    out_x, out_y, ti = test_ori(lane_agent, ori_image, np.array([test_image]), w_ratio, h_ratio,draw_type= 'line',thresh=p.threshold_point)
                # cv2.imwrite(save_test_dir + '/' + "{}_tested.jpg".format(img.split('.jpg')[0]), ti[0])
                img_name = img.split('.jpg')[0]
                write_result_json(out_x,out_y,img_name,save_json_dir,w_ratio,h_ratio)
...
## write result
############################################################################
def write_result_json(x, y,filename,save_path,w_ratio,h_ratio):
    lanes_data = {}
    i=0
    for xi,yi in zip(x[0],y[0]):
        lane = {}
        lane["x"] = [int(x/w_ratio) for x in xi]
        lane["y"] = [int(y/h_ratio) for y in yi]
        lanes_data["lane_"+str(i)] = lane
        i = i+1
    
    json_str = json.dumps(lanes_data)
    #print(json_str)
    #print(save_path + filename + "_lanes_info.json")
    with open(save_path + str(filename) + "_lanes_info.json",'w') as f:
        f.write(json_str)
...
def test(lane_agent, test_images, thresh = p.threshold_point, index= -1):
    ...
    for i in range(num_batch):
        ...
        #save resutl into json file
        # write_result_json(in_x,in_y,'0','./res_json/')  
...
def main(lane_agent,img,img_name,video_name):
    ori_image = img #hw, cv2.IMREAD_UNCHANGED
    test_image = cv2.resize(ori_image, (p.x_size, p.y_size)) / 255.0
    test_image = np.rollaxis(test_image, axis=2, start=0)

    w_ratio = p.x_size * 1.0 / ori_image.shape[1]
    h_ratio = p.y_size* 1.0 / ori_image.shape[0]
    out_x, out_y, ti = test_ori(lane_agent, ori_image, np.array([test_image]),\
     w_ratio, h_ratio,draw_type= 'point',thresh=p.threshold_point)

    save_json_dir = save_root + str(video_name) + '/' + 'lane_json/'
    # print(save_json_dir)
    if not os.path.exists(save_json_dir):
        os.makedirs(save_json_dir)
    res = write_result_json(out_x,out_y,img_name,save_json_dir,w_ratio,h_ratio)

    # save infer image
    save_infer_img_path = save_root + str(video_name) + '/' + 'img_infer_lane'
    if not os.path.exists(save_infer_img_path):
        os.makedirs(save_infer_img_path)
    # cv2.imwrite(save_infer_img_path + '/' + "lane_inferred_{}_.jpg".format(img_name), ti[0])
