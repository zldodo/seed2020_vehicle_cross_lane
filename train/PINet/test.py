...
def Testing():
    ...
    if p.mode == 0 : # check model with test data 
        ...
        cap = cv2.VideoCapture(p.test_root_url+"0.mp4")
        while(cap.isOpened()):
        ...
        # cv2.imshow("test", ti[0])
        # cv2.waitKey(0)   
        test_images = p.test_root_url+'demo0/'
        ...
        for img in img_list[:2]:
            ...
            if use_ori:
                ...
                _, _, ti = test_ori(lane_agent, ori_image, np.array([test_image]), w_ratio, h_ratio,draw_type= 'line',thresh=p.threshold_point)
